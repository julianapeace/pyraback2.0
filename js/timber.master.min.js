...
jQuery('.replicable-content ul > li').wrapInner('<span class="li_content" />');

/** own */
function is_touch_device() {return (('ontouchstart' in window) || (navigator.MaxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0));}if (is_touch_device()) {$('.navigation>li.dropdown').addClass('touch');}else{$('.ddl-switch').css('display','none');}
$('.navigation li.dropdown > a').on('click',function(e){/*e.preventDefault()*/if ($(this).attr('href').indexOf("#") <= 0){$(this).parent().children('ul').css('opacity','0')}});$(".ddl-switch").on("click", function() {$(this).sibling('ul').slideToggle(500);});

if ($.cookie('targacc') !== "") {
	function scrollToAnchor(aid){
		var divTag = $("div[id='"+ aid +"']");
		$('html').animate({scrollTop: divTag.offset().top-320},'slow');
	}
	
	if (window.location.hash) {
		var hash = window.location.hash.substring(1);
		$.cookie('targacc', hash, {expires : 10,path: '/'});
	}
	
	var acclink = '#' + $.cookie('targacc');
	$(acclink).parent('li').addClass("active");
	scrollToAnchor($.cookie('targacc'));
	$.removeCookie("targacc", { path: '/' });
